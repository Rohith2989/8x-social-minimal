...
export const demoCreators = names.map((name, i) => ({ id: i, name, handle: `@${handles[i]}`, poster: previewClips[(i + 4) % previewClips.length].poster, country: i % 5 === 0 ? 'Canada' : 'United States', status: i > 20 ? 'Inactive' : 'Active', workflow: i < 18 ? 'Publishing' : 'In review', joined: 1 + (i % 19) }));
